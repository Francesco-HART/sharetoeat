import { SendNotificationToShopCardsCommand, SendNotificationToShopCardsCommandHandler } from "@app/wallet/application/commands/send-notification-to-shop-cards.command";
...
describe('Send notifications to shop cards', () => {
    describe("Google", () => {
        it('One notification is sent', async () => {
            ...
            const handler = new SendNotificationToShopCardsCommandHandler(cardsRepository);

            await handler.execute(new SendNotificationToShopCardsCommand({
                shopId: "1",
                message: "Hello, world!"
            }));
        })
